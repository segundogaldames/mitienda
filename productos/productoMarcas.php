<?php
    ...
    if (isset($_GET['marca'])) {
        ...
        //consultar por productos asociados a la marca recibida

        //lista de productos con sus imagenes con productos activos e imagenes activas y en portada
        $res = $mbd->prepare("SELECT i.imagen, p.id, p.nombre, p.precio, m.nombre as marca, tp.nombre as tipo FROM imagenes i INNER JOIN productos p ON i.producto_id = p.id INNER JOIN marcas m ON p.marca_id = m.id INNER JOIN producto_tipos tp ON p.producto_tipo_id = tp.id WHERE i.activo = 1 AND i.portada = 1 AND p.activo = 1 AND m.id = ? ORDER BY p.precio");
        $res->bindParam(1, $marca);
        $res->execute();
                ...
                <div class="col-md-9">
                    <div class="row">
                        <?php if(!empty($productos)): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p class="text-info">Sin resultados... Intente con otra marca</p>
                        <?php endif; ?>
                    </div>
                </div>
